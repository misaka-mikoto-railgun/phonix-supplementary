...
아래 수식은 전부 `model.py` 의 `DualObjectiveEQLoss` 에서 그대로 옮긴 것이다. prose 에서
역산하거나 임의로 작성한 항은 없다. 각 식에 정의 심볼을 병기해 코드와 1:1 대조할 수 있게 했다.
손실 함수형은 gain ±6→±12 완화에서 바뀌지 않았다(같은 `a0_proposed_loss` 를 쓴다).

참조는 라인 번호가 아니라 심볼로 적는다. 라인 번호는 코드가 조금만 움직여도 어긋나고,
그러면 "코드와 1:1 대조 가능"이라는 이 문서의 전제부터 무너진다.

코드 출처: `model.py::DualObjectiveEQLoss`
하이퍼파라미터: `train_full.py::build_registry` 의 `a0_proposed_loss`
(A0 이 쓰는 loss 인스턴스이며, A1 · A3 도 같은 인스턴스를 공유한다. A2 는 별도의
`a2_with_pref_loss` 를 쓴다 — 차이는 $\lambda_\text{pref}$ 와 $\lambda_\text{dir}$ 뿐이다.)

---

## 0. 표기 / 그리드
- $\hat{R}\in\mathbb{R}^{F}$: 예측 응답(dB), $T$: 타깃(dB). bin index $k=1,\dots,F$, $F=128$.
- 주파수 그리드: log-spaced $f_k\in[20,\,24000]$ Hz (`DualObjectiveEQLoss.__init__`, `f_max=24000` default = Nyquist). 타깃·예측·loss 그리드 동일.
...
> $\mathcal{L}_\text{room}$은 **동일 weighted-curve 함수형**을 $(\hat{R}_\text{room\,corr},T_\text{room})$에 적용한다(`forward` 안의 room 항). plain MSE/L1 이 아니다.

## 2. Weighted-curve loss (`DualObjectiveEQLoss._weighted_curve_loss`) — final·room 공통
$$
...
$$
(grad 항의 첫 bin $k{=}1$ 은 0 — `target_grad_mag[:, 0] = 0`. 정규화는 per-sample.)

**A-weighting** $w^A_k$ (IEC 61672, 선형진폭 변환 후 평균정규화; `__init__` 의 `a_weight` 버퍼):
$$
...
| $\alpha$ (mag), $\beta$ (grad) | 0.10, 0.30 | `build_registry` → `a0_proposed_loss` |
| edge boost low/high | 1.30 / 1.15 | `DualObjectiveEQLoss.__init__` (default) |
| edge band | $\le300$ / $\ge8000$ Hz | `DualObjectiveEQLoss.__init__` (default) |
| grid | 128 bins, log, 20–24000 Hz | `DualObjectiveEQLoss.__init__` (default) |
